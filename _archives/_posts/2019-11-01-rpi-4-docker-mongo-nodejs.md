...
* sécurité :
  ...
  * accès ssh avec clé uniquement (à noter que ça ne change rien à la connexion en direct sur le rpi) :
    * Créer un couple clé privé/publique avec PuttyGen (avec une passphrase, c'est mieux) ![puttygen]({{ site.baseurl | prepend: site.url }}/wp-content/uploads/puttygen.png)
...
* (optionnel) monter le nas freebox. Ca peut être très intéressant pour configurer un repository git perso, stocker des données, etc. :
 ...
 ![freebox_samba.png]({{ site.baseurl | prepend: site.url }}/wp-content/uploads/freebox_samba.png)
  * config fstab
  ```sh
